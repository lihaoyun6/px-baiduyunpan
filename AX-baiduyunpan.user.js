...
(function(require, define, Promise) {
	...
	function showError(msg) {
		...
		var $dialog = $('<div id="errorDialog">' + '<h3>PX-baiduyunpan:程序异常</h3>' + '<div class="dialog-body"><p>请尝试更新脚本或复制以下信息提交issue</p>' + '<p>Exception: ' + msg + '</p>' + '<p>Script Ver: ' + GM_info.script.version + '</p>' + '<p>TemperMonkey Ver: ' + GM_info.version + '</p>' + '<p>UA: ' + navigator.userAgent + '</p>' + '</div><hr><a class="close" href="javascript:;">关闭</a></div>');
		$dialog.on('click', '.close',
		function(event) {
			$dialog.remove();
		}).appendTo(document.body);
	}
	define('px-yunpan:pageInfo',
	function(require) {
		...
		var PAGE_CONFIG = {
			...
			enterprise: {
				...
				style: function() {
					var styleList = ['.px-yunpan-dropdown-button .icon-download{background-image: url(/box-static/business-function/infos/icons_z.png?t=1476004014313);}', '.px-yunpan-dropdown-button .g-button:hover .icon-download{background-position: 0px -34px;}'];
					GM_addStyle(styleList.join(''));
				}
			}
		};
	...
	define('px-yunpan:downloadBtnInit',
	function(require) {
	...
	define('px-yunpan:pluginInit.js',
	function(require, module, exports) {
		...
		Promise.all([dmPromise, gjcPromise, ddsPromise]).then(function() {
			try {
				...
				ctx.ui.tip({
					mode: 'success',
					msg: 'PX-baiduyunpan: 插件加载成功'
				});
			} catch(e) {
				ctx.ui.tip({
					mode: 'caution',
					msg: 'PX-baiduyunpan: 插件加载成功，按钮初始化失败',
					autoClose: false,
					hasClose: true
				});
			}
